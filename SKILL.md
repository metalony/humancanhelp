...
HumanCanHelp 是一个本地 human-in-the-loop 人工接手工具，适用于 AI 需要真人临时查看实时画面、点击、输入、拖动，或完成受阻步骤的场景。它同时支持浏览器标签页共享（CDP）和桌面共享（VNC）。
...
当前建议优先使用本地仓库运行。如果以后你把这个包发布到自己的环境里，CLI 行为也保持一致：启动 HCL，把打印出的 URL 发给协助者，然后等待 CLI 退出。
...
如果两种来源都不可用，HCL 现在会提前失败并给出设置提示，而不是启动一个实际上无法连接的会话。
...
当 CLI 以退出码 0 结束后，说明人工已经完成了屏幕交互，你就可以继续原来的任务。
...
帮助页面会在以下情况自动过期：
- 达到超时时间 → 页面显示 “This session has expired”，并断开连接
